...
@bot.command(
    name="cancel_meeting",
    help="Specify meeting title and start time in format dd/mm/yyyy HH:MM e.g. !cancel_meeting ML Tutorial 06/03/2021 09:00",
)
        ...
        while True:
            for record in records:
                ...
                if date_time_now==start_time: 
                    title = record[0]
                    userid = record[1]#recheck this 
                    channel_id=record[3]
                    await self.send_message(userid, 'Event "{}" has started!'.format(title))
#need to work on this part yet 
#check if it makes sense to dm users on both the channel or directly on the user_id 
#reference code 
------------------------------------------------------------------------------------------------                    
